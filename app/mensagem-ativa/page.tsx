...
function ActiveMessageAnalytics({ history }: { history: ActiveMessageSendHistory[] }) {
    const [analyticsHistory, setAnalyticsHistory] = useState<ActiveMessageAnalyticsHistoryItem[]>([]);
    const [analyticsLoading, setAnalyticsLoading] = useState(true);
    const [analyticsError, setAnalyticsError] = useState<string | null>(null);
    const { period, setPeriod, selectedRange, setSelectedRange, ready: dateFilterReady } = useDashboardDateFilter("30");
    ...
    useEffect(() => {
        ...
        async function loadAnalytics() {
            try {
                const response = await fetch(`/api/mensagem-ativa/analytics?${analyticsQuery}`, { credentials: "include", cache: "no-store", signal: controller.signal });
                const json = (await response.json()) as ActiveMessageAnalyticsResponse | { error?: string };
                if (!response.ok) throw new Error("error" in json && json.error ? json.error : "Não foi possível carregar o desempenho dos envios");
                setAnalyticsHistory((json as ActiveMessageAnalyticsResponse).history);
            } catch (error) {
                if (controller.signal.aborted) return;
                console.error("[mensagem-ativa] failed to load analytics", error);
                setAnalyticsError(error instanceof Error ? error.message : "Não foi possível carregar o desempenho dos envios");
            } finally {
    ...
    const initialAnalyticsLoading = analyticsLoading && analyticsHistory.length === 0;
    return (
...
function HistoryTable({ history }: { history: ActiveMessageSendHistory[] }) {
    return <div className="mt-10"><SharedHistoryTable history={history} /></div>;
}
