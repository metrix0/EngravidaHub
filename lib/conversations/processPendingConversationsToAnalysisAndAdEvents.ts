...
async function processConversation(conversation: Conversation) {
    ...
    try {
        ...
        // A previous paid/model call may already have saved an analysis while the
        // final conversation-link update failed. Recover that row without calling AI.
        ...
        // Groq-only staged quality pipeline. No OpenAI fallback and no pipeline-level retry.
...
async function claimConversation(conversationId: string) {
    ...
    if (result.error) {
        throw new Error(`Failed to claim conversation: ${result.error.message}`);
    }
...
async function findExistingAnalysis(
    ...
    const result = await supabase
        ...
        .maybeSingle();
    if (result.error) {
        throw new Error(`Failed to check existing analysis: ${result.error.message}`);
    }
...
async function completeConversation(
    ...
    if (result.error) {
        throw new Error(`Failed to complete conversation analysis: ${result.error.message}`);
    }
    if (result.data !== true) {
        throw new Error("Conversation analysis completion was rejected by the database state guard");
    }
}
...
async function failConversation(conversationId: string, error: string) {
    ...
    if (result.error) {
        console.error("[analysis-pipeline] failed to persist permanent failure", {
...
async function sendAdsSafely(
    ...
    let google = null;
    const errors: string[] = [];
    if (!events.length || await hasExistingAdEvents(conversation.id)) {
    ...
    try {
        ...
        try {
            ...
            errors.push(`Meta: ${formatError(error)}`);
        }
        try {
...
async function hasExistingAdEvents(conversationId: string) {
    try {
        ...
        console.error("[analysis-pipeline] ad idempotency check failed", {
            conversation_id: conversationId,
            error,
        });
        return true;
...
async function getPending({
    ...
    if (conversationIds) {
        ...
        return rows
            .sort(
                (a, b) => new Date(a.ended_at ?? a.started_at).getTime()
...
async function getMessages(conversationId: string): Promise<Message[]> {
    ...
    if (result.error) {
        throw new Error(`Failed to fetch conversation messages: ${result.error.message}`);
    }
...
function buildText(messages: Message[]) {
    return messages
        .map(
            (message) => `[${new Date(message.sent_at).toLocaleString("pt-BR")}] ${senderLabel(message)}: ${message.text}`,
        )
        .join("\n");
}
...
function concurrency() {
    const value = Number(
        process.env.CONVERSATION_ANALYSIS_CONCURRENCY ?? DEFAULT_CONCURRENCY,
    );
    return Math.min(
        MAX_CONCURRENCY,
        Math.max(1, Number.isFinite(value) ? Math.floor(value) : DEFAULT_CONCURRENCY),
    );
}
